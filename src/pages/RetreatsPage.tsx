  ...
  return (
    <div className="min-h-screen bg-[#f3eee6] text-forest-deep"><Header /><main>
      <section className="pt-28 md:pt-32"><div className="container-soft"><Reveal className="grid gap-0 md:grid-cols-[0.9fr_1.1fr] border border-forest/10 bg-[#dbe5df]">
        <div className="order-2 md:order-1 p-8 md:p-12 bg-[#dbe5df] flex flex-col justify-start min-h-[390px] md:min-h-[560px]"><div><p className="eyebrow text-forest/55 mb-5">{copy.eyebrow}</p><h1 className="font-serif text-6xl md:text-8xl leading-none tracking-[-0.03em]">{copy.title}</h1><p className="font-serif italic text-2xl md:text-4xl text-forest/72 mt-3 max-w-xl text-balance">{copy.subtitle}</p></div><div className="mt-8 md:mt-10 space-y-1.5 text-base md:text-lg leading-relaxed text-forest/80 max-w-xl text-pretty">{copy.intro.map(p => <p key={p}>{p}</p>)}</div></div>
      ...
      <section className="py-12 md:py-16"><div className="container-soft grid gap-0 md:grid-cols-2 border border-forest/10 bg-[#f4f0e8]"><Reveal><MediaSpace label={copy.image} /></Reveal><Reveal className="p-8 md:p-12 flex items-center"><div className="space-y-6 text-base md:text-[17px] leading-[1.75] text-forest/80 max-w-xl text-pretty"><p>{copy.first[0]}</p><p className="font-serif italic text-lg md:text-xl leading-relaxed text-forest-deep text-balance">{copy.first[1]}</p></div></Reveal></div></section>

      <section className="pb-12 md:pb-16"><div className="container-soft grid gap-0 md:grid-cols-[0.72fr_1.28fr] border border-forest/10"><Reveal className="bg-[#b9cfcb] p-8 md:p-12 flex items-center min-h-[360px]"><div className="space-y-5 text-base md:text-lg leading-relaxed text-forest-deep/82 text-pretty"><p className="font-serif text-2xl md:text-3xl text-balance">{copy.second[0]}</p><p className="font-serif italic text-xl md:text-2xl text-balance">{copy.second[1]}</p></div></Reveal><Reveal><MediaSpace label={copy.video} video dark /></Reveal></div></section>

      <section className="bg-[#4d4942] text-ivory py-14 md:py-20"><div className="container-soft"><Reveal className="max-w-4xl mx-auto space-y-6 text-base md:text-lg leading-[1.8] text-ivory/84 text-pretty">{copy.second.slice(2).map((p,i) => <p key={p} className={i === 2 ? "font-serif italic text-xl md:text-2xl text-[#d7c3aa] text-balance" : ""}>{p}</p>)}</Reveal></div></section>

      <section className="py-14 md:py-20 bg-[#f3eee6]"><div className="container-soft"><Reveal className="max-w-3xl mx-auto text-center mb-12 md:mb-16"><p className="eyebrow text-[#7c5d4d] mb-4">{copy.exploreEyebrow}</p><h2 className="font-serif text-3xl md:text-5xl leading-tight mb-4 text-balance">{copy.exploreTitle}</h2><p className="text-sm md:text-base leading-relaxed text-forest/72 max-w-2xl mx-auto text-pretty">{copy.exploreIntro}</p></Reveal>
        <div className="border border-forest/10">{copy.explore.slice(0,-1).map((item,index) => { const reverse=index%2===1; const darkTone=["clay","moss","earth"].includes(item.tone); return <Reveal key={item.title}><div className={`grid md:grid-cols-2 ${reverse ? "md:[&>*:first-child]:order-2" : ""}`}><div className={`${TONES[item.tone]} min-h-[250px] md:min-h-[320px] flex items-center p-8 md:p-11`}><div className="max-w-md"><h3 className={`font-serif text-2xl md:text-3xl leading-tight mb-5 text-balance ${darkTone ? "text-[#f7f1e8]" : "text-forest-deep"}`}>{item.title}</h3><div className={`space-y-3 text-sm md:text-[15px] leading-[1.75] text-pretty ${darkTone ? "text-ivory/82" : "text-forest/78"}`}>{item.paragraphs.map(p => <p key={p}>{p}</p>)}</div></div></div><div className={`${index%3===0 ? "bg-[#e7ece7]" : index%3===1 ? "bg-[#d8c8b4]" : "bg-[#c7d5d1]"} min-h-[250px] md:min-h-[320px] flex items-center justify-center p-8`}><div className="w-2/3 md:w-3/5 aspect-[4/5] border border-forest/10 bg-[#f4f0e8] flex items-center justify-center text-center"><span className="eyebrow text-forest/40">{copy.image}</span></div></div></div></Reveal>})}</div>
        <Reveal className="mt-14 md:mt-16"><div className="grid md:grid-cols-[0.36fr_0.64fr] border border-forest/10 bg-[#c8aa8d]"><div className="bg-[#7a6654] text-ivory p-8 md:p-10 flex items-start md:items-center"><div><p className="eyebrow text-ivory/60 mb-3">{copy.homeEyebrow}</p><div className="font-serif text-5xl md:text-7xl leading-none">{copy.homeWord}</div></div></div><div className="p-8 md:p-12"><h3 className="font-serif text-2xl md:text-3xl mb-5 text-balance">{copy.explore[copy.explore.length-1].title}</h3><div className="space-y-3 text-sm md:text-base leading-[1.75] text-forest/82 max-w-2xl text-pretty">{copy.explore[copy.explore.length-1].paragraphs.map(p => <p key={p}>{p}</p>)}</div></div></div></Reveal>
      </div></section>
